...
-- Create improved employee creation function with updated delegation validation
...
BEGIN
  ...
  IF NEW.delegation IS NOT NULL THEN
    ...
    BEGIN
      NEW.delegation := NEW.delegation::delegation_enum;
    EXCEPTION
      WHEN invalid_text_representation THEN
        RAISE EXCEPTION 'Delegación no válida: %. Valores permitidos: MADRID, ALAVA, SANTANDER, SEVILLA, VALLADOLID, MURCIA, BURGOS, PROFESIONALES, ALICANTE, LA, CADIZ, PALENCIA, CORDOBA', NEW.delegation;
    END;
  END IF;
...
-- Create improved supervisor creation function with updated delegation validation
