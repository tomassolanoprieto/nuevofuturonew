...
DROP FUNCTION IF EXISTS get_work_centers_by_delegation(delegation_enum);
DROP FUNCTION IF EXISTS get_filtered_requests_by_delegation(delegation_enum, work_center_enum, TIMESTAMPTZ, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS get_delegation_calendar_events(delegation_enum, work_center_enum, TIMESTAMPTZ, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS get_delegation_work_centers(delegation_enum);

-- Create function to get work centers for a delegation
CREATE OR REPLACE FUNCTION get_delegation_work_centers_v2(p_delegation delegation_enum)
...
CREATE OR REPLACE FUNCTION get_delegation_filtered_requests_v2(
...
BEGIN
  ...
  v_work_centers := get_delegation_work_centers_v2(p_delegation);
...
CREATE OR REPLACE FUNCTION get_delegation_calendar_events_v2(
  p_delegation delegation_enum,
  p_work_center work_center_enum DEFAULT NULL,
  p_start_date TIMESTAMPTZ DEFAULT NULL,
  p_end_date TIMESTAMPTZ DEFAULT NULL
...
-- Create function to get work centers list for a delegation
CREATE OR REPLACE FUNCTION get_delegation_work_centers_list_v2(
  p_delegation delegation_enum
)
RETURNS SETOF work_center_enum AS $$
  SELECT unnest(get_delegation_work_centers_v2(p_delegation))
  ORDER BY 1;
...
GRANT EXECUTE ON FUNCTION get_delegation_work_centers_v2 TO PUBLIC;
GRANT EXECUTE ON FUNCTION get_delegation_filtered_requests_v2 TO PUBLIC;
GRANT EXECUTE ON FUNCTION get_delegation_calendar_events_v2 TO PUBLIC;
GRANT EXECUTE ON FUNCTION get_delegation_work_centers_list_v2 TO PUBLIC;