-- First backup existing data as text
...
SELECT 
    ...
    created_at,
    updated_at,
    array_to_string(work_centers::text[], ',') as work_centers_text,
    job_positions,
    seniority_date
...
SELECT 
    ...
    document_number,
    array_to_string(work_centers::text[], ',') as work_centers_text,
    array_to_string(delegations::text[], ',') as delegations_text,
    supervisor_type,
    pin,
    employee_id,
    created_at,
...
SELECT 
    ...
    eb.document_number,
    CASE 
        WHEN eb.delegation_text = 'LA' THEN 'CONCEPCION_LA'::delegation_enum
        WHEN eb.delegation_text = 'CONCEPCION (LA)' THEN 'CONCEPCION_LA'::delegation_enum
    ...
    ARRAY(
        SELECT DISTINCT 
            CASE 
                WHEN wc = 'LA LINEA CAI / CARMEN HERRERO' THEN 'CONCEPCION_LA LINEA CAI / CARMEN HERRERO'
                WHEN wc = 'LA LINEA ESPIGON' THEN 'CONCEPCION_LA LINEA ESPIGON'
                WHEN wc = 'LA LINEA MATILDE GALVEZ' THEN 'CONCEPCION_LA LINEA MATILDE GALVEZ'
                WHEN wc = 'LA LINEA GIBRALTAR' THEN 'CONCEPCION_LA LINEA GIBRALTAR'
                WHEN wc = 'LA LINEA EL ROSARIO' THEN 'CONCEPCION_LA LINEA EL ROSARIO'
                WHEN wc = 'LA LINEA PUNTO DE ENCUENTRO' THEN 'CONCEPCION_LA LINEA PUNTO DE ENCUENTRO'
                WHEN wc = 'LA LINEA SOROLLA' THEN 'CONCEPCION_LA LINEA SOROLLA'
                ELSE wc
...
SELECT 
    ...
    ARRAY(
        SELECT DISTINCT 
            CASE 
                WHEN d = 'LA' THEN 'CONCEPCION_LA'::delegation_enum
                WHEN d = 'CONCEPCION (LA)' THEN 'CONCEPCION_LA'::delegation_enum
...
BEGIN
    ...
    RETURN CASE v_prefix
        ...
        WHEN 'CONCEPCION_LA' THEN 'CONCEPCION_LA'
