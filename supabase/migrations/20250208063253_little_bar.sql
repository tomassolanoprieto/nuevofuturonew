...
DO $$ 
BEGIN
    -- Drop policies on employee_profiles
    DROP POLICY IF EXISTS "supervisor_center_employee_access_v2" ON employee_profiles;
    DROP POLICY IF EXISTS "supervisor_center_employee_access_v3" ON employee_profiles;
    DROP POLICY IF EXISTS "supervisor_center_employee_access_v4" ON employee_profiles;
    
    -- Drop policies on time_entries
...
CREATE POLICY "supervisor_center_employee_access_v5"
...
CREATE POLICY "supervisor_center_time_entries_v5"
  ON time_entries
  FOR ALL
  TO authenticated
  USING (
...
CREATE OR REPLACE FUNCTION get_supervisor_center_employees_v3(p_supervisor_id UUID)
RETURNS TABLE (
  employee_id UUID,
  fiscal_name TEXT,
  email TEXT,
...
CREATE OR REPLACE FUNCTION get_supervisor_center_time_entries_v3(p_supervisor_id UUID)
RETURNS TABLE (
  entry_id UUID,
  emp_id UUID,
  entry_type TEXT,
