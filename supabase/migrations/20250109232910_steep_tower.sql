/*
  # Fix employee authentication final v3

  1. Changes
    - Drop and recreate trigger with improved error handling
    - Update auth handling to ensure proper user creation
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = NEW.id 
    AND raw_app_meta_data->>'role' = 'employee'
  ) THEN
    UPDATE auth.users 
...
CREATE TRIGGER handle_employee_auth_trigger
  BEFORE INSERT OR UPDATE OF email, pin ON employee_profiles
  FOR EACH ROW
