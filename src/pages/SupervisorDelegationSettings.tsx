...
const delegationOptions = ["MADRID"];
...
const fetchSupervisors = async () => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('company_id', user.id) // Filtrar por empresa
      .order('created_at', { ascending: false });

    if (error) throw error;

    // Filtrar los supervisores en el frontend
    const filteredSupervisors = data.filter(supervisor => {
      if (!supervisor.work_centers) return false;

      // Verificar si alguno de los centros de trabajo coincide con workCenterOptions
      return supervisor.work_centers.some(center => workCenterOptions.includes(center));
    });
