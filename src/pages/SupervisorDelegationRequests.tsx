...
function SupervisorDelegationRequests() {
  const [requests, setRequests] = useState<Request[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filter, setFilter] = useState<'all' | 'pending' | 'approved' | 'rejected'>('pending');
  const [supervisorDelegation, setSupervisorDelegation] = useState<string>('');
  ...
  useEffect(() => {
    const getSupervisorInfo = async () => {
      try {
        ...
        if (!supervisorEmail) {
          throw new Error('No se encontró el correo electrónico del supervisor en localStorage');
        }

        // Obtener la delegación del supervisor desde Supabase
        ...
        // Guardar la delegación y obtener las solicitudes
        setSupervisorDelegation(supervisor.delegations);
        fetchRequests(supervisor.delegations, startDate, endDate);
      } catch (error) {
  ...
  const fetchRequests = async (delegation: string, startDate: string, endDate: string) => {
    try {
      setLoading(true);

      // Llamar a la función get_filtered_requests con la delegación
      const { data: requests, error } = await supabase.rpc(
        'get_filtered_requests_by_delegation',
        { 
  ...
  const handleExportExcel = () => {
    ...
    const filename = `solicitudes_${supervisorDelegation.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Delegación: {supervisorDelegation}
          </p>
        </div>
...
export default SupervisorDelegationRequests;